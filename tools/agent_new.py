...
SERVER_PORT=8899
DETECT_TIME=60
EMAIL_LIST=['shitian@s.xxxx.com','alert@s.xxx.com']
LOG_FILE='/tmp/agent.log'
LOG_ERR_FILE='/tmp/agent_error.log'
...
def sendsms(mess):
    app_id=['5680f043xxxx-ed40-ab09-6750f859cf78']
...
def main():
    ...
    parse.add_option('-o','--operation',
	    action='store',
	    help='run or stop this script options: start|stop',
	    dest='operation',
	    type='string',
	    )

    opts,args=parse.parse_args()
    if not opts.server_ip or not opts.operation:
	parse.print_help()
	sys.exit(1)
	...
	if os.path.exists(PID_FILE):
	    print 'pid file exists ! the agentd is running ?'
	    sys.exit(1)
